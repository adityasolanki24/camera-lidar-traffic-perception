...
def visualize_training_results(train_loss_list, train_acc_list, val_loss_list, val_acc_list, best_acc, optimizer, learning_rate, batch_size, scheduler_type, plot_dir='results'):
    """
    ...
    Args:
        ...
        plot_dir (str): Directory for saved PNG files
    """
    ...
    out_path = os.path.join(plot_dir, 'training_results.png')
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    print(f"Saved training curves to {os.path.abspath(out_path)}")
    plt.show()


def visualize_predictions(model, test_loader, class_names, device, num_samples=16, plot_dir='results'):
    """
    ...
    os.makedirs(plot_dir, exist_ok=True)
    out_path = os.path.join(plot_dir, 'predictions.png')
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    print(f"Saved prediction grid to {os.path.abspath(out_path)}")
    plt.show()


def plot_confusion_matrix(model, test_loader, class_names, device, criterion, plot_dir='results'):
    """
    ...
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    print(f"Saved confusion matrix to {os.path.abspath(out_path)}")
    plt.show()
